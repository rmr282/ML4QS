...
data = sensor_readouts.set_index('timestamp', drop=True)
data = data.join(pd.get_dummies(data.label, prefix='label'))

data.to_csv(FILENAME + '_raw.csv')

selected_predictor_cols = [c for c in data.columns if not 'label' in c]
...
data = NumAbs.abstract_numerical(data, selected_predictor_cols, ws, 'mean')
data = NumAbs.abstract_numerical(data, selected_predictor_cols, ws, 'std')
data = CatAbs.abstract_categorical(data, ['label_id'], ['like'], 0.03, int(float(5*60000)/delta), 2)

data = FreqAbs.abstract_frequency(copy.deepcopy(data), periodic_measurements, int(float(10000) / delta), fs)
...
sensor_data = sensors.get_group('LSM6DSM Accelerometer').copy()
sensor_data['value'] = sensor_data['value'].apply(lambda x: literal_eval(x))
sensor_data = sensor_data.join(pd.DataFrame(sensor_data['value'].to_list(), columns=['x', 'y', 'z'], index=sensor_data.index))


mask = np.ones(len(sensor_data), dtype=bool)
last_ts = sensor_data.iloc[0].timestamp
for i, (_, ts) in enumerate(sensor_data.timestamp[1:].iteritems()):
...
#         copy.deepcopy(lin_acc), ['x', 'y', 'z'], k, 'default', 20, n_inits=10)
...
dataset_kmed = clusteringNH.k_medoids_over_instances(
    copy.deepcopy(sensor_data), ['x', 'y', 'z'], k, 'default', 20, n_inits=50)
